...
class Firebase {
  ...
  community = (userUid, communityUid) =>
      this.user(userUid).collection('communities').
          doc(`${communityUid}`);
  ...
  doGetUserCommunities = () =>
      this.communities(this.auth.currentUser.uid);

  doGetUserCommunity = (uid) =>
      this.community(this.auth.currentUser.uid, uid);
  ...
  doAddCommunity = (payload) => {
    ...
    return this.doGetUserCommunities().add({
      ...payload,
    }).then((docRef) => {
      ...
      this.allCommunities().add({
        userUid: this.auth.currentUser.uid,
        communityUid: docRef.id,
      }).then((communityRef) => {
        Log.success('New community logged with user+community ID\'s',
            this.auth.currentUser.uid, communityRef.id);
      }).catch((error) => {
  ...
  doRemoveCommunity = (uid) => {
    this.doGetUserCommunity(uid).delete().then(() => {
